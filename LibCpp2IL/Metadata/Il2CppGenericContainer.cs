...
public class Il2CppGenericContainer : ReadableClass
{
    ...
    public IEnumerable<Il2CppGenericParameter> GenericParameters
    {
        get
        {
            ...
            for (var i = 0; i < genericParameterCount; i++)
            {
                ...
                var p = OwningMetadata!.GetGenericParameterFromIndex(index);
                p.Index = index;
    ...
    public Il2CppTypeDefinition? TypeOwner => isGenericMethod ? null : OwningMetadata!.GetTypeDefinitionFromIndex(Il2CppVariableWidthIndex<Il2CppTypeDefinition>.MakeTemporaryForFixedWidthUsage(ownerIndex));

    //DynWidth: ownerIndex is always int, so making temp is ok
    public Il2CppMethodDefinition? MethodOwner => isGenericMethod ? OwningMetadata!.GetMethodDefinitionFromIndex(Il2CppVariableWidthIndex<Il2CppMethodDefinition>.MakeTemporaryForFixedWidthUsage(ownerIndex)) : null;
