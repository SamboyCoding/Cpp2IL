...
public class Il2CppAssemblyNameDefinition : ReadableClass
{
    ...
    public string Culture => OwningMetadata!.GetStringFromIndex(cultureIndex);
    ...
    public byte[]? PublicKey
    {
        get
        {
            ...
            if (IsAtLeast(24.4f) || Is(24.15f))
            {
                ...
                var result = OwningMetadata!.GetByteArrayFromIndex(publicKeyIndex);
                return result.Length == 0 ? null : result;
            }
            else
            {
                var str = OwningMetadata!.GetStringFromIndex(publicKeyIndex);
    ...
    public string HashValue => MetadataVersion > 24.3f ? "NULL" : OwningMetadata!.GetStringFromIndex(hashValueIndex);
